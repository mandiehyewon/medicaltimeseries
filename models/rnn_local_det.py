...
class RNN_LOCAL_DET(nn.Module):
    def __init__(self,config):
        super(RNN_LOCAL_DET,self).__init__()
        self.module_dict = nn.ModuleDict()

        self.config = config
        
        self.module_dict['rnn'] = CUSTOM_RNN(input_size=config.input_dim,
                                    hidden_size=config.hidden_dim, num_layers=config.num_layers, dropout=config.dropout)
...
class CUSTOM_RNN(nn.Module):
    ...
    def forward(self,input):
        ...
        for i in range(self.num_layers):
            ...
            c_concat, h_concat, h  = self.module_dict[str(i)](data,None)
